    ...
    useEffect( () => {
        const getFilmDetails = async () => {
            const response = await fetch(
                `https://api.themoviedb.org/3/movie/${id}?api_key=${tmdbKey}&append_to_response=credits,images,keywords` )
                ...
                <Row>
                    <Col sm={12} md={4} lg={4} className="py-2">
                        <img src={film.poster_path
                            ...
                            : `https://dummyimage.com/w500/222/fff.png&text=NA+poster`}
                            alt={film.title}
                    ...
                    <Col sm={12} md={8} lg={8} className="py-2">
                        <div className="details__inner">
                            ...
                            {film.genres && film.genres.length > 0
                                ? film.genres.map( genre => <span key={genre.id}>
                                    {genre ? genre.name : ""} {" "}</span> )
                                : ""
                            }
                            <br />
                            <span>
                                {film.production_countries ? film.production_countries[0].name : ""} / {film.release_date} / {film.runtime}{""}min
                                </span>
                            <span>
